...
interface RunScreeningDialogProps {
  open: boolean;
  vendorName: string;
  onClose: () => void;
...
export function RunScreeningDialog({ open, vendorName, onClose, onConfirm }: RunScreeningDialogProps) {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      setErr(e instanceof Error ? e.message : 'Failed to start screening');
    } finally {
      setSubmitting(false);
    }
  };
        ...
        >
          <div className="flex items-start justify-between p-6 border-b border-sand-100">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h2 className="font-serif text-lg text-rose-900">Run New Screening</h2>
          ...
          <div className="p-6 space-y-3">
            <p className="text-sm font-sans text-sand-700">
              Launches a fresh financial health and security assessment. Previous screenings are preserved in the history.
            </p>
            {err && (
