...
export class MockJourneyService extends BaseMockService implements IJourneyService {
  ...
  async submitFeedback(_journeyId: string, data: { mood: number; reflection?: string; emotionalTags?: string[] }) {
    return { id: this.generateId(), journeyId: _journeyId, userId: '', mood: data.mood, reflection: data.reflection, emotionalTags: data.emotionalTags || [], createdAt: this.now() };
  }

  async getFeedback(_journeyId: string) {
    return [];
  }

  async setPreDepartureBrief(_journeyId: string, data: { summary: string; logistics?: Record<string, unknown>; advisorNotes?: string; autoGenerated?: boolean }) {
    return { id: this.generateId(), journeyId: _journeyId, summary: data.summary, logistics: data.logistics, advisorNotes: data.advisorNotes, autoGenerated: data.autoGenerated ?? false, createdAt: this.now() };
  }

  async getTravelMonitor(_journeyId: string) {
    return null;
  }
}
